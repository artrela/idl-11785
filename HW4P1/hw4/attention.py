...
class Attention:
        ...
        def forward(self, X):
            ...
            # @ operator works on the last two dims (https://discuss.pytorch.org/t/how-does-the-sign-work-in-this-instance/11232)
            self.A_w    = self.Q @ torch.transpose(self.K, 2, 1)

            # Create additive causal attention mask and apply mask
            # Hint: Look into torch.tril/torch.triu and account for batch dimension
            attn_mask    = torch.tril(self.A_w, 0)
            attn_mask[attn_mask == 0 ] = -1e9
            
            # Calculate/normalize Attention Scores
            self.A_sig   = self.softmax.forward( attn_mask / (self.D_k**(0.5)) )
        ...
        def backward(self, dLdXnew):
            ...
            dLdA_sig       = dLdXnew @ torch.transpose(self.V, 2, 1)
            dLdA_w         = ( self.D_k**(-0.5) ) * self.softmax.backward(dLdA_sig)

            # Derivatives wrt keys, queries, and value
            
            self.dLdV      = torch.transpose(self.A_sig, 2, 1) @ dLdXnew
            self.dLdK      = torch.transpose(dLdA_w, 2, 1) @ self.Q
            ...
            self.dLdWq     = torch.sum(torch.transpose(self.X, 2, 1) @ self.dLdQ,  dim=0) 
            self.dLdWv     = torch.sum(torch.transpose(self.X, 2, 1) @ self.dLdV,  dim=0)
            self.dLdWk     = torch.sum(torch.transpose(self.X, 2, 1) @ self.dLdK,  dim=0)

            # Derivative wrt input

            dLdX      = self.dLdV @ torch.transpose(self.W_v, 1, 0) + \
                        self.dLdK @ torch.transpose(self.W_k, 1, 0) + \
                        self.dLdQ @ torch.transpose(self.W_q, 1, 0)
